...
import com.laps.lapsfood.domain.repository.RestauranteRepositoryQueries;
...
public class RestauranteRepositoryImpl implements RestauranteRepositoryQueries{
	...
	public List<Restaurante> consultar(String nome, BigDecimal taxaFreteInicial, BigDecimal taxaFreteFinal){
		var jqpl = new StringBuilder("from Restaurante where 1=1 ");
		...
		if(taxaFreteInicial != null) {
			jqpl.append("and taxaFrete >= :taxaFreteInicial ");
			parametros.put("taxaFreteInicial", taxaFreteInicial);
		}
		
		if(taxaFreteFinal != null) {
			jqpl.append("and taxaFrete <= :taxaFreteFinal ");
			parametros.put("taxaFreteFinal",taxaFreteFinal);
		}
		
		var query = manager.createQuery(jqpl.toString(), Restaurante.class);
	...
	public List<Restaurante> consultarCriteria(String nome, BigDecimal taxaInicial, BigDecimal taxaFinal){
		...
		Predicate taxaFinalPredicate = builder.lessThanOrEqualTo(root.get("taxaFrete"), taxaInicial);
